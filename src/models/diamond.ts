...
export class Diamond {
  ...
  private chanceOfNeutral = 0.1;
  private chanceOfColorChange = 0.9;
  ...
  private updateColors(): void {
    ...
    const leftVariation = Math.random() * baseMultiplier - leftToRightVariation;
    const rightVariation =
      Math.random() * baseMultiplier - leftToRightVariation;
    ...
    this.sections.bottomright.style.borderBottomColor = `hsl(${hue}, ${saturation}%, ${clampLightness(leftBaseLightness - topBottomVariation)}%)`;
    this.sections.topright.style.borderTopColor = `hsl(${hue}, ${saturation}%, ${clampLightness(leftBaseLightness + topBottomVariation)}%)`;

    this.sections.bottomleft.style.borderBottomColor = `hsl(${hue}, ${saturation}%, ${clampLightness(rightBaseLightness - topBottomVariation)}%)`;
    this.sections.topleft.style.borderTopColor = `hsl(${hue}, ${saturation}%, ${clampLightness(rightBaseLightness + topBottomVariation)}%)`;
  }
  ...
  private startColorAnimation(): void {
    const animate = (): void => {
      ...
      if (!isChangingColor) {
        setTimeout(
          () => animate(),
          Math.random() * 6000 + this.intervalColorTransition,
        );
        return;
      }
